...
const state = {
  ...
  favs: JSON.parse(localStorage.getItem("uplift_favs") || "[]"),
...
function renderFavList() {
  ...
  list.innerHTML = state.favs.length
    ? state.favs.map((f, i) =>
        `<div class="fav-item"><p>"${f.text}"</p><span>— ${f.author}</span>
         <button class="rm-fav" onclick="removeFav(${i})"><i class="fas fa-times"></i></button></div>`
      ).join("")
...
function removeFav(i) {
  ...
  localStorage.setItem("uplift_favs", JSON.stringify(state.favs));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("shareBtn").addEventListener("click", () => {
    ...
    if (navigator.share) navigator.share({ title: "Uplift", text });
      ...
      setTimeout(() => btn.innerHTML = '<i class="fas fa-share-alt"></i>', 1800);
    });
  });
  ...
  document.getElementById("clearFavs").addEventListener("click", () => {
    ...
    state.favs = [];
    localStorage.setItem("uplift_favs", "[]");
